<form action="/modif/user/<?php echo $user->id();?>" method="POST" class="col-xs-6 ">
    ...
    <div class="has-feedback form-group">
        <label class="col-sm-12 control-label" for="rank">R&ocirc;le&nbsp;:</label>
        <div class="col-sm-12">
            <select style="width: 300px;" name="rank">
                <?php
                $rankList = Rank::getAllRanks();
                foreach ($rankList as $rank) {
                    if ($rank->name() != "Super Administrateur") {
                        $select = "";
                        if ($rank->id() == $user->rank()->id())
                            $select = "selected";
                        echo "<option $select value='" . $rank->id() . "'>" . $rank->name() . "</option>";
                    }
                }
                ?>
            </select>
